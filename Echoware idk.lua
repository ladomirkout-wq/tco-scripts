...
local ACTIVE_USER_FILE = "echoware_active.dat"
local AGREEMENT_FILE = "" -- will be set per username
local LICENSE_FILE = ""   -- will be set per username
local SLOT_FILE = ""      -- will be set per username
local BOTS_FILE = ""      -- will be set per username
local MSG_FILE = ""       -- will be set per username

local currentUsername = ""

local function setUserFiles(username)
    currentUsername = username
    LICENSE_FILE = "echoware_license_" .. username .. ".dat"
    SLOT_FILE = "echoware_slot_" .. username .. ".dat"
    AGREEMENT_FILE = "echoware_agreed_" .. username .. ".dat"
    BOTS_FILE = "echoware_bots_" .. username .. ".dat"
    MSG_FILE = "echoware_msg_" .. username .. ".dat"
end
...
function main(allowedUsername, slotNumber)
    ...
    task.spawn(function()
        ...
        if messageToggles.loading then
            sendChat("Echoware | Credits to Echo and Kingnoob")
        end
    end)
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    elseif formationType == "wall" then
                        ...
                        pos = targetRoot.Position + right * (col * 4) + Vector3.new(0, row * 4, 0)
                        orientCFrame = CFrame.lookAt(pos, pos + look)
                        myRoot.CFrame = orientCFrame
                    elseif formationType == "tower" then
                        ...
                        local row = level
                        pos = targetRoot.Position - look * 2 + right * (col * 3) + Vector3.new(0, row * 4, 0)
                        orientCFrame = CFrame.lookAt(pos, pos + look)
                        myRoot.CFrame = orientCFrame
    ...
    local function processCommand(sender, message)
        ...
        elseif string.sub(clean, 1, #Prefix + #Wall + 1) == Prefix .. Wall .. " " then
            local tgtText = string.sub(message, string.find(message, Wall .. " ") + #Wall + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then
                startFormation(tgt, "wall", false)
                if messageToggles.formation then sendChat("formation:wall (" .. (tgtText or tgt.Name) .. ")") end
            end
        elseif cmd == Prefix .. Wall then
            startFormation(sender, "wall", false)
        ...
        elseif string.sub(clean, 1, #Prefix + #Tower + 1) == Prefix .. Tower .. " " then
            local tgtText = string.sub(message, string.find(message, Tower .. " ") + #Tower + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then
                startFormation(tgt, "tower", false)
                if messageToggles.formation then sendChat("formation:tower (" .. (tgtText or tgt.Name) .. ")") end
            end
        elseif cmd == Prefix .. Tower then
            startFormation(sender, "tower", false)
        ...
        elseif string.sub(clean, 1, #Prefix + #Dlh + 1) == Prefix .. Dlh .. " " then
            local tgtText = string.sub(message, string.find(message, Dlh .. " ") + #Dlh + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then
                startFormation(tgt, "dlh", false)
                if messageToggles.formation then sendChat("formation:dlh (" .. (tgtText or tgt.Name) .. ")") end
            end
        elseif cmd == Prefix .. Dlh then
            startFormation(sender, "dlh", false)
        ...
        elseif string.sub(clean, 1, #Prefix + #Follow + 1) == Prefix .. Follow .. " " then
            local tgtText = string.sub(message, string.find(message, Follow .. " ") + #Follow + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then
                stopAllMovement()
