...
function main(allowedUsername, slotNumber)
    ...
    function startCommands(AllowedUser, slotNumber)
        ...
        local function lineTeleport(target)
            local myChar = LocalPlayer.Character
            local targetChar = target.Character
            if myChar and targetChar then
                local myRoot = myChar:FindFirstChild("HumanoidRootPart")
        ...
        local allCommands = ".say .loopsay .stoploop .dall .adall .stopadall .silent .fling .stopfling .hide .stophide .reset .rejoin .antiafk .crash .follow .orbit .lineup .stopmove .mod .removemod .alert .credits .cmds .bots .shutdown .orbitspeed .raid .line .prefix"

        local function processCommand(sender, message)
            local clean = message:lower():gsub("^;%.", ""):gsub("^;", "")
            if clean == "" then clean = message:lower() end
            if string.sub(clean, 1, #Prefix) == Prefix then
                clean = Prefix .. clean:sub(#Prefix+1)
            ...
            elseif string.sub(message, 1, #Prefix + #Follow + 1) == Prefix .. Follow .. " " then
            ...
            elseif cmd == Prefix .. Follow then startFollow(sender)
            ...
            elseif cmd == Prefix .. Lineup then startLineup(sender)
            ...
            elseif string.sub(message, 1, #Prefix + #Line + 1) == Prefix .. Line .. " " then
                ...
                local tgtText = string.sub(message, startPos + #Line + 1)
                local tgt = getTarget(tgtText)
                if tgt then lineTeleport(tgt) end
            elseif cmd == Prefix .. Line then lineTeleport(sender)
