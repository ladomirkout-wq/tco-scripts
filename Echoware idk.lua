...
if queue_on_teleport then
    queue_on_teleport([[
        if not _G.EchowareUnloaded then
...
function main(allowedUsername, slotNumber)
    ...
    -- Modified equipTool: takes a quiet parameter (default false)
    local function equipTool(toolName, quiet)
        ...
        -- Search backpack case-insensitively
        local found = nil
    ...
    -- Helper to unequip all tools from character
    ...
    -- FIXED: Circle now uses target's CFrame (angle)
    ...
    -- FIXED: Star now uses target's CFrame (angle)
    local function getStarPosition(leaderRoot, slot, total, size)
        ...
        for i = 1, #points do
            if targetDist <= acc + distances[i] then
                ...
                -- FIXED: Use leaderRoot.CFrame to transform local offset to world space
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    if formationType == "line" then
                        -- Line already uses target's LookVector/RightVector (copies angle)
                    ...
                    elseif formationType == "lineup" then
                        -- Lineup already uses target's LookVector (copies angle)
                        local look = targetRoot.CFrame.LookVector
                        pos = targetRoot.Position - look * (slotNumber * LINEUP_SPACING)
                        orientCFrame = CFrame.lookAt(pos, pos + look)
                        myRoot.CFrame = orientCFrame
                    elseif formationType == "circle" then
                        -- FIXED: Circle uses target's CFrame for rotation
                    ...
                    elseif formationType == "star" then
                        -- FIXED: Star uses target's CFrame for rotation
    ...
    -- FIXED: Orbit now uses target's CFrame (angle)
        ...
        orbitConnection = RunService.Heartbeat:Connect(function(dt)
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    -- FIXED: Convert local offset to world space using target's CFrame
    ...
    -- Animation override (silent, aggressive)
    ...
    local function setAnimationOverride(state)
        ...
        if state then
            -- Start heartbeat loop to keep animations frozen
                    ...
                    pcall(function()
                        ...
                        if char then
                            ...
                            if hum then
                                ...
                                if anim then
                                    -- Set all tracks to speed 0 and detach animator
            ...
            -- Stop loop and restore animations
            ...
            pcall(function()
                ...
                if char then
                    local hum = char:FindFirstChildOfClass("Humanoid")
                    if hum then
                        -- Find orphaned Animator (maybe in character or game)
    ...
    local allCommands = ".say .loopsay .stoploop .dall .adall .stopadall .silent .fling .stopfling .hide .stophide .reset .rejoin .antiafk .crash .form .stopform .line .circle .orbit .lineup .star .stopmove .mod .removemod .alert .credits .cmds .bots .botscheck .mb .raidcalc .unload .shutdown .orbitspeed .raid .prefix .antilag .equip .animations"
    ...
    local function processCommand(sender, message)
        ...
        if cmd == Prefix .. Unload then
            -- Cancel queue teleport by setting global flag
            _G.EchowareUnloaded = true
            cleanupConnections()
        ...
        elseif string.sub(clean, 1, #Prefix + #Form + 1) == Prefix .. Form .. " " then
            ...
            if #args >= 1 then
                ...
                if tgt then
                    if formType == "line" or formType == "circle" or formType == "lineup" or formType == "star" then
        ...
        elseif cmd == Prefix .. RaidCalc then
            ...
            sendChat("RaidCalc: " .. mins .. " minutes and " .. secs .. " seconds to reach " .. target .. " time with " .. totalBots .. " bots + owner.")
        ...
        -- .equip command (silent, multi/single)
        elseif string.sub(clean, 1, #Prefix + #Equip + 1) == Prefix .. Equip .. " " then
            local startPos = string.find(message, Equip .. " ", 1, true)
            if startPos then
                local argsText = string.sub(message, startPos + #Equip + 1):gsub("^%s+", ""):gsub("%s+$", "")
                if argsText == "" then
                    -- no items, do nothing
                elseif string.find(argsText, ",") then
                    -- multi-item
                    local items = {}
                    for item in argsText:gmatch("[^,]+") do
                    ...
                    -- single item: unequip all first
                    unequipAllTools()
                    equipTool(argsText, true)
                end
            end
        -- .animations command (silent, swapped on/off)
        elseif string.sub(clean, 1, #Prefix + #Animations + 1) == Prefix .. Animations .. " " then
            local startPos = string.find(message, Animations .. " ", 1, true)
            local arg = ""
            if startPos then
                arg = string.sub(message, startPos + #Animations + 1):gsub("^%s+", ""):gsub("%s+$", ""):lower()
            end
            if arg == "on" then
                setAnimationOverride(false) -- restore normal animations
            elseif arg == "off" then
                setAnimationOverride(true) -- freeze animations
            end
            -- no chat message
