...
pcall(function() local content = readfile(AGREEMENT_FILE) if content then hasAgreed = true end end)

local function startAfterSetup()
...
local function startAfterKey()
    if hasAgreed then
        startAfterSetup()
    else
        showTermsPrompt(startAfterSetup)
    end
end
...
function main(allowedUsername, slotNumber)
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    if formationType == "line" then
                        pos = getDividedPosition(targetRoot, slotNumber, totalBots, FOLLOW_DISTANCE)
    ...
    local allCommands = ".say .loopsay .stoploop .dall .adall .stopadall .silent .fling .stopfling .hide .stophide .reset .rejoin .antiafk .crash .form .stopform .line .circle .lineup .square .stopmove .mod .removemod .alert .credits .cmds .bots .shutdown .orbitspeed .raid .prefix"
