...
local backupAnimateScript = nil
local animOverrideActive = false

local function hardOverrideFreeze()
    local character = LocalPlayer.Character
    if not character then return end
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if not humanoid then return end
    local animator = humanoid:FindFirstChildOfClass("Animator")
    local coreAnimate = character:FindFirstChild("Animate")
    if coreAnimate and coreAnimate:IsA("LocalScript") then
        backupAnimateScript = coreAnimate:Clone()
        coreAnimate:Destroy()
    end
    if animator then
...
local function restoreAnimationSystem()
    local character = LocalPlayer.Character
    if not character then return end
    if not character:FindFirstChild("Animate") then
        if backupAnimateScript then
            local restoredScript = backupAnimateScript:Clone()
            restoredScript.Parent = character
        else
            local humanoid = character:FindFirstChildOfClass("Humanoid")
...
-- ANTI-LAG (hides everything except EchoPlatform)
local autoMatchMsg = "°"
...
local function hidePart(part)
    if part:IsA("BasePart") and part.Name ~= "EchoPlatform" then
        part.Transparency = 1
    end
end
...
local function hideAllParts()
    for _, desc in ipairs(workspace:GetDescendants()) do
        if desc:IsA("BasePart") and desc.Name ~= "EchoPlatform" then
            desc.Transparency = 1
        end
...
function main(allowedUsername, slotNumber)
    ...
    local ownerName = ""
    if allowedUsername and allowedUsername ~= "" then hasOwner = true; ownerName = allowedUsername end
    ...
    local hiding, hidePart, hidePos, returnPos = false, nil, nil, nil
    ...
    local orbitAngle = 0
    local orbitInitialAngle = 0
    local movementPlatform = nil
    local orbitTarget = nil
    local currentTarget = nil
    local orbitResetTask = nil
    ...
    local STAR_SIZE = 10
    local TOWER_BEHIND_DISTANCE = 3
    local TOWER_LAYER_HEIGHT = 5
    local TOWER_SQUARE_SIZE = 4
    ...
    local function crashSequence()
        stopAllMovement()
        for i = 1, 5 do sendChat("clone a") task.wait(0.1) end
        sendChat("freeze a")
        task.wait(0.1)
        sendChat("tospawn a")
        task.wait(0.1)
    ...
    local function stopAllMovement()
        ...
        if orbitResetTask then orbitResetTask:Cancel(); orbitResetTask = nil end
    ...
    local function getStarPosition(leaderRoot, slot, total, size)
        ...
        for i = 1, #points do
            if targetDist <= acc + distances[i] then
                ...
                pos = leaderRoot.Position + localPos
                break
            end
            acc = acc + distances[i]
        end
    ...
    local function getTowerPosition(leaderRoot, slot, total)
        local layer = math.floor((slot - 1) / 4)
        local inLayer = (slot - 1) % 4
        local look = leaderRoot.CFrame.LookVector
        local right = leaderRoot.CFrame.RightVector
        local basePos = leaderRoot.Position - look * TOWER_BEHIND_DISTANCE + Vector3.new(0, layer * TOWER_LAYER_HEIGHT, 0)
        local half = TOWER_SQUARE_SIZE / 2
        local offset
        if inLayer == 0 then
            offset = right * -half + look * -half
        elseif inLayer == 1 then
            offset = right * half + look * -half
        elseif inLayer == 2 then
            offset = right * -half + look * half
        else
            offset = right * half + look * half
        end
    ...
    local function startFormation(target, formationType, follow)
        ...
        orbitTarget = nil
        if follow or formationType == "tower" then createPlatform() end
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    if formationType == "line" then
                        ...
                        orientCFrame = CFrame.new(pos) * targetRoot.CFrame.Rotation
                    ...
                    elseif formationType == "tower" then
                        pos = getTowerPosition(targetRoot, slotNumber, totalBots)
                        local look = targetRoot.CFrame.LookVector
                        local basePos = targetRoot.Position - look * TOWER_BEHIND_DISTANCE + Vector3.new(0, math.floor((slotNumber - 1) / 4) * TOWER_LAYER_HEIGHT, 0)
                        orientCFrame = CFrame.lookAt(pos, basePos)
                    end
                    if pos and orientCFrame then
                        myRoot.CFrame = orientCFrame
                    end
                    if movementPlatform and myRoot then
                        movementPlatform.CFrame = CFrame.new(myRoot.Position - Vector3.new(0, 2.5, 0)) * myRoot.CFrame.Rotation
                    end
                end
            end
        end
    ...
    local function startOrbit(target)
        ...
        orbitConnection = RunService.Heartbeat:Connect(function(dt)
            ...
            if myChar and targetChar then
                ...
                if myRoot and targetRoot then
                    ...
                    local orientCFrame = CFrame.new(newPos) * targetRoot.CFrame.Rotation
                    myRoot.CFrame = orientCFrame
                    if movementPlatform then
                        movementPlatform.CFrame = CFrame.new(myRoot.Position - Vector3.new(0, 2.5, 0)) * myRoot.CFrame.Rotation
                    end
        ...
        if orbitResetTask then orbitResetTask:Cancel() end
        orbitResetTask = task.spawn(function()
            while orbitConnection do
                task.wait(60)
                orbitAngle = orbitInitialAngle
            end
        end)
    end
    ...
    RunService.Heartbeat:Connect(function()
        if hiding then
            ...
            if char then char:PivotTo(CFrame.new(hidePos)) end
        end
    end)

    local allCommands = ".say .loopsay .stoploop .dall .adall .stopadall .silent .fling .stopfling .hide .stophide .reset .rejoin .antiafk .crash .form .stopform .line .circle .orbit .lineup .star .tower .equip .animation .stopmove .mod .removemod .alert .credits .cmds .bots .botscheck .mb .unload .shutdown .orbitspeed .raid .prefix .antilag"
    ...
    local function processCommand(sender, message)
        ...
        if cmd == Prefix .. AntiLag .. " off" then disableAntiLag(); sendChat("disabled")
        elseif cmd == Prefix .. AntiLag then applyAntiLag(); sendChat("enabled")
        elseif cmd == Prefix .. AntiAfk then loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-ANTI-AFK-by-gun-265109"))(); sendChat("Anti AFK loaded")
        elseif cmd == Prefix .. AntiAfk .. " off" then sendChat("Anti AFK off not supported")
        elseif cmd == Prefix .. Crash then crashSequence()
        elseif cmd == Prefix .. Silent then
            if string.sub(cmd, #Prefix + #Silent + 2) == "on" then silentMode = true
            elseif string.sub(cmd, #Prefix + #Silent + 2) == "off" then silentMode = false
            else silentMode = not silentMode end
            sendChat("Silent mode " .. (silentMode and "on" or "off"))
        elseif cmd == Prefix .. Rejoin then pcall(function() TeleportService:Teleport(game.PlaceId, LocalPlayer) end)
        elseif cmd == Prefix .. Animation then
            local sub = string.match(cmd, "^" .. Prefix .. Animation .. "%s+(%w+)")
            if sub == "off" then hardOverrideFreeze(); sendChat("Animations overridden (frozen)")
            elseif sub == "on" then restoreAnimationSystem(); sendChat("Animations restored")
            else sendChat("Usage: .animation off/on") end
        elseif cmd == Prefix .. Equip then
            local itemName = string.sub(cmd, #Prefix + #Equip + 2)
            if itemName ~= "" then equipTool(itemName); sendChat("Equipped " .. itemName) end
        elseif cmd == Prefix .. Reset then
            ...
            if char then
                ...
                if hum then
                    ...
                    brick.Touched:Connect(function(hit) if hit.Parent == char then hum.Health = 0 end end)
                    task.delay(3, function() if hum and hum.Parent then hum.Health = 0 end end)
                    hum.Died:Wait()
                    brick:Destroy()
                end
            end
        elseif cmd == Prefix .. Hide then
            if not hiding then
                ...
                if char then
                    ...
                    local randomY = math.random(5000, 10000)
                    hidePos = Vector3.new(randomX, randomY, randomZ)
                    char:PivotTo(CFrame.new(hidePos))
                end
        ...
        elseif cmd == Prefix .. StopForm or cmd == Prefix .. StopMove then stopAllMovement()
        elseif string.sub(message, 1, #Prefix + #Form + 1) == Prefix .. Form .. " " then
            ...
            if #args >= 1 then
                ...
                local tgtText = nil
                for i = 2, #args do
                    if args[i]:lower() == "follow" then follow = true else tgtText = args[i] end
                end
                local tgt = getTarget(tgtText or sender.Name)
                if tgt then
                    if formType == "line" or formType == "circle" or formType == "lineup" or formType == "star" or formType == "tower" then
                        startFormation(tgt, formType, follow)
                    end
                end
            end
        elseif cmd == Prefix .. Form then startFormation(sender, "line", false)
        ...
        elseif string.sub(message, 1, #Prefix + #Orbit + 1) == Prefix .. Orbit .. " " then
            ...
            local tgt = getTarget(tgtText or (currentTarget and currentTarget.Name or sender.Name))
            if tgt then startOrbit(tgt) end
        elseif cmd == Prefix .. Orbit then
            local tgt = currentTarget or sender
            if tgt then startOrbit(tgt) end
        ...
        elseif string.sub(message, 1, #Prefix + #Star + 1) == Prefix .. Star .. " " then
            local startPos = string.find(message, Star .. " ")
            local tgtText = string.sub(message, startPos + #Star + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local follow = string.find(tgtText, "follow") ~= nil
            tgtText = tgtText:gsub("follow", ""):gsub("%s+$", ""):gsub("^%s+", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then startFormation(tgt, "star", follow) end
        elseif cmd == Prefix .. Star then startFormation(sender, "star", false)
        elseif string.sub(message, 1, #Prefix + #Tower + 1) == Prefix .. Tower .. " " then
            local startPos = string.find(message, Tower .. " ")
            local tgtText = string.sub(message, startPos + #Tower + 1):gsub("^%s+", ""):gsub("%s+$", "")
            local follow = string.find(tgtText, "follow") ~= nil
            tgtText = tgtText:gsub("follow", ""):gsub("%s+$", ""):gsub("^%s+", "")
            local tgt = getTarget(tgtText or sender.Name)
            if tgt then startFormation(tgt, "tower", follow) end
        elseif cmd == Prefix .. Tower then startFormation(sender, "tower", false)
        elseif string.sub(message, 1, #Prefix + #Dall + 1) == Prefix .. Dall .. " " then
            ...
            local t = getOwnTime()
            if t and t.Value > 1 then sendChat(";donate " .. fixUsername(target.Name) .. " " .. (t.Value - 1)) end
        ...
        elseif string.sub(message, 1, #Prefix + #Adall + 1) == Prefix .. Adall .. " " then
            ...
            local target = sender
            if #args >= 1 then
                if tonumber(args[1]) then interval = tonumber(args[1]); if #args >= 2 then target = getTarget(args[2]) or sender end
                else target = getTarget(args[1]) or sender; if #args >= 2 and tonumber(args[2]) then interval = tonumber(args[2]) end end
            end
        ...
        elseif cmd == Prefix .. BotsCheck then sendChat(autoMatchMsg)
        elseif string.sub(message, 1, #Prefix + #MB + 1) == Prefix .. MB .. " " then
            ...
            if msg ~= "" then autoMatchMsg = msg; sendChat("Auto-match message set to: " .. msg) end
        elseif cmd == Prefix .. MB then autoMatchMsg = "°"; sendChat("Auto-match message reset to °")
        ...
        elseif string.sub(message, 1, #Prefix + #RemoveMod + 1) == Prefix .. RemoveMod .. " " then
            ...
            if targetText:lower() == "a" then ModUsers = {}; sendChat("All mods removed")
            else
                local target = getTarget(targetText or sender.Name)
                if target then
        ...
        elseif cmd == Prefix .. RemoveMod then ModUsers[string.lower(sender.DisplayName)] = nil
        ...
        elseif string.sub(message, 1, #Prefix + #OrbitSpeed + 1) == Prefix .. OrbitSpeed .. " " then
            ...
            if num and num > 0 then orbitSpeed = num; sendChat("Orbit speed set to " .. num) end
        ...
        elseif string.sub(message, 1, #Prefix + #PrefixCmd + 1) == Prefix .. PrefixCmd .. " " then
            ...
            if newPrefix and newPrefix ~= "" then
                ...
                if #newPrefix > 0 then Prefix = newPrefix end
            end
        end
    end
...
setSlotBtn.MouseButton1Click:Connect(function()
    ...
    if num and num >= 1 and num <= 99 then
        ...
        if savedUser then saveData(savedUser, num); main(savedUser, num)
        else showUsernamePrompt(function(username) saveData(username, num); main(username, num) end) end
    end
end)
...
local function performSetup()
    if not isLicenseValid() then showKeyPrompt(function() performSetup() end); return end
    local agreed = false
    pcall(function() local content = readfile(AGREEMENT_FILE) if content then agreed = true end end)
    if not agreed then showTermsPrompt(function() performSetup() end); return end
    local savedUser, savedSlot = loadSavedData()
