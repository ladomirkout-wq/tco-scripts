...
function main(allowedUsername, slotNumber)
    ...
    -- Animation override with heartbeat loop (swapped on/off)
    ...
    local function processCommand(sender, message)
        ...
        -- New .equip command: multi-item support, silent execution, unequip previous for single item
        elseif string.sub(clean, 1, #Prefix + #Equip + 1) == Prefix .. Equip .. " " then
            ...
            if startPos then
                ...
                if argsText == "" then
                    -- No items specified, do nothing silently
                elseif string.find(argsText, ",") then
                    -- Multi-item: split by comma, equip each (no unequip)
                    local items = {}
                    ...
                    for _, item in ipairs(items) do
                        if item ~= "" then
                            equipTool(item, true) -- silent
                        end
                    end
                else
                    -- Single item: unequip all tools first, then equip this item
                    unequipAllTools()
                    equipTool(argsText, true) -- silent
                end
            end
        -- New .animations command (swapped on/off)
        elseif string.sub(clean, 1, #Prefix + #Animations + 1) == Prefix .. Animations .. " " then
            ...
            if arg == "on" then
                setAnimationOverride(false) -- ON means restore normal animations (turn off freeze)
                sendChat("Animations enabled (normal)")
            elseif arg == "off" then
                setAnimationOverride(true) -- OFF means freeze animations (turn on override)
                sendChat("Animations frozen")
            else
                sendChat("Usage: .animations on/off")
            end
        elseif cmd == Prefix .. Animations then
            setAnimationOverride(not animOverrideActive)
            sendChat(animOverrideActive and "Animations frozen" or "Animations enabled (normal)")
        end
    end
