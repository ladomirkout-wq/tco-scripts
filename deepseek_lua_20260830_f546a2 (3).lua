...
function main(allowedUsername, slotNumber)
    ...
    local function getSquarePosition(leaderRoot, slot, total, size)
        ...
        local dist = ((slot - 1) / total) * (4 * size)
        local localX, localZ
        
        if dist <= size then
            localX = -half + dist
            localZ = -half
        elseif dist <= 2 * size then
            localX = half
            localZ = -half + (dist - size)
        elseif dist <= 3 * size then
            localX = half - (dist - 2 * size)
            localZ = half
        else
            localX = -half
            localZ = half - (dist - 3 * size)
        end
        
        return leaderRoot.Position + right * localX + look * localZ
    end
    ...
    local allCommands = ".say .loopsay .stoploop .dall .adall .stopadall .silent .fling .stopfling .hide .stophide .reset .rejoin .antiafk .crash .form .stopform .line .circle .orbit .lineup .square .star .stopmove .mod .removemod .alert .credits .cmds .bots .botscheck .mb .unload .shutdown .orbitspeed .raid .prefix .antilag"
    ...
    local function processCommand(sender, message)
        ...
        elseif string.sub(message, 1, #Prefix + #Form + 1) == Prefix .. Form .. " " then
            ...
            if #args >= 1 then
                ...
                if tgt then
                    if formType == "line" or formType == "circle" or formType == "lineup" or formType == "square" or formType == "star" then
